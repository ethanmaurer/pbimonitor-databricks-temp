...
# MAGIC %md
# MAGIC # Load Capacity Users
# MAGIC This notebook will read a json file from the storage account and append to the transformed.capacities_users table.
# MAGIC
# MAGIC It will read the json using the schema from ./011_schema from a path that looks similar to `/apiData/raw/capacities_users_getCapacityUsers/2023/06/30/1688164245/capacity-users.json`
# MAGIC
...
readPath = f"/mnt/data-warehouse/apiData/raw/{topic}_getCapacityUsers/{processedDate}/{timestamp}/*.json"
...
df = spark.read.json(readPath, capacityUsersSchema, multiLine=True)
